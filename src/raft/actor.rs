...
use crate::utils::client::{connect_raft_client, execute_with_redirect};
...
pub enum ActorMsg {
    ...
    Status {
        reply_to: oneshot::Sender<ActorStatus>,
    },
...
impl RaftActor {
    pub fn new(
        ...
        if let Ok(content) = std::fs::read_to_string(&snapshot_path) {
            if let Ok(snapshot) = serde_json::from_str::<Snapshot>(&content) {
                if snapshot.last_included_index == state.last_included_index {
                    ...
                    warn!(
                        "Ignoring uncommitted snapshot {} at index {}",
                        snapshot_path.display(),
                        snapshot.last_included_index
                    );
                }
                ...
                warn!(
                    "Failed to deserialize snapshot from {}",
                    snapshot_path.display()
                );
            }
        } else {
        ...
        let replay_to = state.commit_index.min(state.last_log_index());
        for index in (state.last_included_index + 1)..=replay_to {
            let physical_idx = (index - state.last_included_index) as usize;
            if let Some(entry) = state.log.get(physical_idx) {
                let command = entry.command.clone();
                Self::apply_membership_to_state(&mut state, &command);
    ...
    async fn persist_state(&self) -> Result<(), NodeError> {
        ...
        let path = self.state.hard_state_path();

        spawn_blocking(move || RaftState::save_hs_to_path(hs, path))
            .await
    ...
    /* Message Handlers */
    async fn handle_message(&mut self, msg: ActorMsg, election_timer: &mut Interval) {
        match msg {
            ...
            ActorMsg::Status { reply_to } => {
                let role = match self.state.role {
                    Role::Follower => "follower",
                    Role::Candidate => "candidate",
                    Role::Leader => "leader",
                };
                let ready = self.state.role == Role::Leader || self.state.current_leader.is_some();
                let _ = reply_to.send(ActorStatus {
                    node_id: self.state.my_id,
                    role: role.to_string(),
                    term: self.state.current_term,
                    leader_id: self.state.current_leader,
                    commit_index: self.state.commit_index,
                    last_applied: self.state.last_applied,
                    configured_peers: self.state.peers.len(),
    ...
    async fn handle_install_snapshot(
        ...
        if let Ok(snapshot) = serde_json::from_slice::<Snapshot>(&data) {
            ...
            if let Err(e) = tokio::fs::rename(&tmp_path, &snapshot_path).await {
                warn!("Failed to activate installed snapshot: {}", e);
                let _ = reply_to.send(InstallSnapshotReply {
                    term: self.state.current_term,
                    success: false,
                });
    ...
    async fn apply_committed_entries(&mut self) {
        ...
        while self.state.last_applied < self.state.commit_index {
            ...
            if physical_idx < log_len {
                ...
                Self::apply_membership_to_state(&mut self.state, &command);
                ...
                if let Some(sender) = self.pending_requests.remove(&idx) {
                    completed_requests.push((sender, result));
                }
            } else {
        ...
        match self.persist_state().await {
            Ok(()) => {
                for (sender, result) in completed_requests {
                    let _ = sender.send(Ok(result));
                }
            }
            Err(e) => {
                let message = format!("Failed to persist applied state: {}", e);
                error!("{}", message);
                for (sender, _) in completed_requests {
                    let _ = sender.send(Err(NodeError::Internal(message.clone())));
                }
            }
        }
    }

    fn apply_membership_to_state(state: &mut RaftState, command: &Command) {
        match command {
            Command::AddNode { id, address } if *id != state.my_id => {
                state.peers.insert(*id, address.clone());
                if state.role == Role::Leader {
                    let next = state.last_log_index() + 1;
                    state.next_index.entry(*id).or_insert(next);
                    state.match_index.entry(*id).or_insert(0);
                }
            }
    ...
    async fn start_election(&mut self) {
        ...
        let (tx, mut rx): (
            mpsc::Sender<(u64, RequestVoteReply)>,
            mpsc::Receiver<(u64, RequestVoteReply)>,
        ) = mpsc::channel(self.state.peers.len().max(1));

        for (peer_id, peer_addr) in self.state.peers.clone() {
            let existing_client = self.peers.get(&peer_id).cloned();
            let tx_inner = tx.clone();
            let actor_sender = self.msg_sender.clone();
            ...
            tokio::spawn(async move {
                let client = match existing_client {
                    ...
                    .await
                    {
                        ...
                        _ => {
                            warn!("Could not connect to peer {} for election", peer_id);
                            return;
                        }
                    },
                };
        ...
        if votes_received >= majority {
            info!(
                "Won single-node election. Becoming LEADER for Term {}",
                term
            );
                    ...
                    let client = match existing_client {
                        ...
                        None => {
                            match tokio::time::timeout(
                                ...
                                connect_raft_client(&peer_addr_str),
                            )
                            .await
                            {
                                Ok(Ok(new_client)) => {
    ...
    async fn handle_apply_membership(
        ...
        self.state
            .next_index
            .insert(node_id, self.state.last_log_index() + 1);
        ...
        match connect_raft_client(&node_addr).await {
            Ok(client) => {
                self.peers.insert(node_id, client);
            ...
            Err(e) => {
                ...
                tokio::spawn(async move {
                    loop {
                        ...
                        if let Ok(client) = connect_raft_client(&retry_addr).await {
                            let _ = msg_sender
                                .send(ActorMsg::UpdatePeerClient { node_id, client })
                                .await;
                            break;
                        }
                    }
        ...
        let response = ApplyMembershipResponse { peers: final_peers };
        let _ = reply_to.send(Ok(response));
    }
