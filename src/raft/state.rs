...
impl RaftState {
    pub fn new(id: u64, peers: HashMap<u64, String>) -> Self {
        Self::new_with_config(id, peers, format!("127.0.0.1:{}", 8000 + id), ".")
    }
    ...
    pub fn new_with_config(
        ...
        let storage_path = storage_dir.join(format!("node_{}.json", id));
        if let Ok(hs) = Self::load_hs(&storage_path) {
            info!("Loaded persistent state from {}", storage_path.display());
            return Self::restore(id, peers, my_addr, storage_dir, hs);
        }
    ...
    pub fn hard_state_path(&self) -> PathBuf {
        self.storage_dir.join(format!("node_{}.json", self.my_id))
    }

    pub fn snapshot_path(&self) -> PathBuf {
        self.storage_dir
            .join(format!("snapshot_{}.json", self.my_id))
    }

    pub fn save_hs_to_disk(hs: HardState, id: u64) -> Result<(), NodeError> {
        Self::save_hs_to_path(hs, PathBuf::from(format!("node_{}.json", id)))
    }

    pub fn save_hs_to_path(hs: HardState, path: PathBuf) -> Result<(), NodeError> {
        if let Some(parent) = path.parent() {
            create_dir_all(parent).map_err(|e| {
                NodeError::Internal(format!("Create storage directory error: {}", e))
            })?;
        }
    ...
    fn load_hs(path: &Path) -> Result<HardState, NodeError> {
        if !path.exists() {
    ...
    fn restore(
        ...
        let mut peers = hs.peers.clone();
        peers.extend(configured_peers);
        peers.remove(&id);
